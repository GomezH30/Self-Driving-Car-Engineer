...
int main ()
{
  ...
  **/
  ...
  double kp_steer = 0.3;
  double kd_steer = 0.5;
  double ki_steer = 0.002;
  double lim_max_steer = 1.2;
  double lim_min_steer = -1.2;
  
  double kp_throttle = 0.2;
  double kd_throttle = 0.025;
  double ki_throttle = 0.001;
  double lim_max_throttle = 1.0;
  double lim_min_throttle = -1.0;

  PID pid_steer = PID();
  pid_steer.Init(kp_steer, ki_steer, kd_steer, lim_max_steer, lim_min_steer);
    
  PID pid_throttle = PID();
  pid_throttle.Init(kp_throttle, ki_throttle, kd_throttle, lim_max_throttle, lim_min_throttle);
